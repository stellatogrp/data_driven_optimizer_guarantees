...
from opt_guarantees.algo_steps import (
    k_steps_eval_fista,
    k_steps_eval_lista_cpss,
    k_steps_train_lista_cpss,
)
...
class LISTA_CPSSmodel(L2WSmodel):
    def __init__(self, **kwargs):
        super(LISTA_CPSSmodel, self).__init__(**kwargs)

    def initialize_algo(self, input_dict):
        self.factor_static = None
        self.algo = 'lista_cpss'
        ...
        self.k_steps_train_fn = partial(k_steps_train_lista_cpss, D=D,
                                        jit=self.jit)
        self.k_steps_eval_fn = partial(k_steps_eval_lista_cpss, D=D,
                                       jit=self.jit)
        ...
        self.W_1_tensor = jnp.tile(self.W_1[:, :, jnp.newaxis], (1, 1, self.train_unrolls))
        # self.W_2_tensor = jnp.tile(self.W_2[:, :, jnp.newaxis], (1, 1, self.train_unrolls))

    def init_params(self):
        # self.mean_params = (jnp.ones((self.train_unrolls, 2)), 
        #                         jnp.ones((self.m, self.n)))
        self.mean_params = (self.lambd / self.L * jnp.ones((self.train_unrolls)), #jnp.ones((self.train_unrolls)), 
                            self.W_1_tensor + 0.0000001 #.001
                            )
        self.sigma_params = (-jnp.ones((self.train_unrolls)) * 10, 
                                -jnp.ones((self.m, self.n, self.train_unrolls)) * 10)
    ...
    def create_end2end_loss_fn(self, bypass_nn, diff_required):
        ...
        def predict(params, input, q, iters, z_star, key, factor):
            ...
            perturb2 = random.normal(w_key, (self.m, self.n, self.train_unrolls))
            # perturb3 = random.normal(w_key, (self.n, self.n, self.train_unrolls))
            ...
            if diff_required:
                ...
                penalty_loss = 0 #calculate_pinsker_penalty(self.N_train, params, self.b, self.c, 
                                                    #  self.delta,
                                                    #  prior=self.W)
